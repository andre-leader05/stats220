...
The day with the highest rate was **`r days[which.max(avg_week)]`** with an average rate of **`r round(max_day_num,2)`**.

Our sample size was very small, and so we cannot claim with certainty that **`r days[which.max(avg_week)]`** tends to show the highest number of Toyota Aquas on the road. However, based on our sample data, Toyota Aquas were spotted more frequently on **`r max_day`** on average. With a larger sample size, it would be worth testing the statistical significance of the difference between the **`max_day`** average and the mean average rate of Aquas per minute.
