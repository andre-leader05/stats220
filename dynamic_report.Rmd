...
Does the day of the week influence the rate of Toyota Aquas that are spotted? Participants of the survey kept track of the number of Toyota Aquas they spotted while driving, and the duration of their trip.
...
The above barplot counts the amount of observations (trips in the car) that were made on each day of the week. Although our sample size is very limited, with only two observations on Wednesday, we have data for each day of the week and can comment on hour our data differs past on the day!
...
The day with the highest rate was **`r days[which.max(avg_week)]`** with an average rate of **`r round(max(avg_week),2)`**.

Our sample size was very small, and so we cannot claim with certainty that **`r days[which.max(avg_week)]`** tends to show the highest number of Toyota Aquas on the road. However, based on our sample data, Toyota Aquas were spotted more frequently on **`r days[which.max(avg_week)]`** on average. With a larger sample size, it would be worth testing the statistical significance of the difference between the **`r days[which.max(avg_week)]`** average and the mean average rate of Aquas per minute.

## What were the most common Toyota Aqua colours?

Below is a plot displaying the number of observations that each colour of Toyota Aqua were spotted. The bars are colour coded to match the car colour represented.
...
main_colour_number <- max(table(data_longer_cars$car_colours))
```

The Toyota Aqua colour spotted in the most trips was **`r main_colour`** which was spotted in **`r main_colour_number`** trips.
